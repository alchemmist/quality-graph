...
class Project:
    ...
    def generate(self) -> GeneratedProject:
        ...
        self._validate_output_paths(generated)
        prettier_ignore = self._prettier_ignore(generated)
        retired = self._retired_paths(generated)
        for path in retired:
            if (path.exists() or path.is_symlink()) and (
                ...
                message = f"Refusing to remove an unmanaged retired workflow: {path}"
    ...
    def _validate_output_paths(self, generated: GeneratedProject) -> None:
        root = self.root.resolve()
        for relative in (*[item.path for item in generated.files], PRETTIER_IGNORE_PATH):
            path = self.root / relative
            if root not in path.resolve().parents:
                message = f"Generated file is outside the project root: {path}"
                raise ValueError(message)
            if path.is_symlink():
                message = f"Refusing to overwrite a generated-file symbolic link: {path}"
                raise ValueError(message)
            if (
                self.graph.provider.name == "gitlab"
                and relative.suffix in {".yml", ".yaml"}
                and path.is_file()
                and not path.read_text().startswith(
                    "# Generated by Quality Graph. DO NOT EDIT MANUALLY."
                )
            ):
                message = (
                    f"Refusing to overwrite unmanaged GitLab CI: {path}; "
                    "use ci-path: .qg/gitlab-ci.yml and include it"
                )
                raise FileExistsError(message)

    def _retired_paths(self, generated: GeneratedProject) -> tuple[Path, ...]:
        root = self.root.resolve()
        required = tuple(self.root / path for path in generated.retired_files)
        optional = tuple(self.root / path for path in generated.retired_if_generated)
        for path in (*required, *optional):
            if root not in path.resolve().parents:
                message = f"Retired workflow is outside the project root: {path}"
                raise ValueError(message)
        owned = tuple(
            path
            ...
            and path.is_file()
            and path.read_text().startswith("# Generated by Quality Graph. DO NOT EDIT MANUALLY.")
        )
    ...
    def validate(self) -> ValidationReport:
        ...
        self._validate_output_paths(generated)
        problems.extend(
            f"retired generated file: {path.relative_to(self.root)}"
            for path in self._retired_paths(generated)
    ...
    def initialize(
        ...
        *,
        ...
        """Create a backwards-compatible GitHub starter declaration."""
    ...
    def initialize_provider(
        ...
        *,
        ...
        """Initialize a provider that supplies its own starter declaration."""
        provider = load_provider(provider_name)
        if not isinstance(provider, ProviderInitializer):
